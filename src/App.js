...
// ============================================
// INTRANET DE EMPLEADOS - APLICACIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN PRINCIPAL
// v1.3 - Horarios conectados
// ============================================
// Esta aplicaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n se conecta a la misma base de datos Supabase
// del sistema principal. Los usuarios ingresan con su nÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºmero
// de documento (cÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©dula) y la misma clave del sistema principal.

// ConfiguraciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de Supabase desde archivo config.js
...
function App() {
  ...
  // Verificar si hay sesiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n guardada al cargar
  useEffect(() => {
  ...
  const cargarDatosEmpleado = async (usuarioData) => {
    ...
    try {
      ...
      if (emp && !error) {
        ...
        // Guardar el ID del empleado para buscar nÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³minas
        const empleadoId = emp.id || emp.documento;
        console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‹Å“Ãƒâ€šÃ‚Â¤ Empleado encontrado, ID:', empleadoId, 'Documento:', emp.documento);
        
        // Cargar configuraciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de empresa
        ...
        // Cargar datos adicionales usando ID para nÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³minas y horarios, documento para el resto
        await Promise.all([
  ...
  const cargarNominas = async (empleadoId, documento) => {
    try {
      console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒâ€šÃ‚Â Buscando nÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³minas para empleadoId:', empleadoId, 'documento:', documento);
      
      // PRIMERO: Ver cuÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ntas nÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³minas hay en total en la tabla
      const { data: todasNominas, count, error: errorTotal } = await supabase
        .from('nominas')
        .select('id, empleadoid, periodo, totalneto', { count: 'exact' })
        .limit(10);
      
      console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€¦Ã‚Â  TOTAL nÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³minas en tabla:', count, 'Primeras 10:', todasNominas, errorTotal);
      
      // Mostrar los empleadoid para debug
      if (todasNominas && todasNominas.length > 0) {
        console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‹Å“Ãƒâ€šÃ‚Â¥ Empleadoids en la tabla:', todasNominas.map(n => n.empleadoid));
      }
      ...
      console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¹ Resultado bÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºsqueda por empleadoid (ID):', data?.length || 0, error);
      
      // Si no encuentra por ID, intentar por documento
      if ((!data || data.length === 0) && !error && documento) {
        console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾ Intentando bÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºsqueda por documento...');
        ...
        console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¹ Resultado bÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºsqueda por documento:', dataDoc?.length || 0);
        if (dataDoc && dataDoc.length > 0) {
          data = dataDoc;
        }
      }
      
      // Si aÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºn no encuentra, buscar con ilike por si hay prefijos/sufijos
      if ((!data || data.length === 0) && !error) {
        console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾ Intentando bÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºsqueda con contains...');
        ...
        console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¹ Resultado bÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºsqueda ilike:', dataAlt?.length || 0);
      ...
      if (data) {
        console.log('ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã¢â‚¬Å“ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ NÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³minas encontradas:', data.length);
        setNominas(data);
      }
    } catch (e) {
      console.log('ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦Ã¢â‚¬â„¢ Error cargando nÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³minas:', e);
    }
  };

  const cargarHorarios = async (empleadoId) => {
    try {
      console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ Buscando horarios para empleado ID:', empleadoId);
      ...
      console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â  Buscando horarios desde', fechaInicio, 'hasta', fechaFin);
      ...
      console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ Horarios encontrados:', horariosData?.length || 0);
        ...
        for (const semana of horariosData) {
          ...
          if (horarioEmpleado) {
            ...
            for (const [diaIndex, turno] of Object.entries(horarioEmpleado)) {
              ...
              // Solo incluir si estÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ dentro del rango
      ...
      // Ordenar por fecha descendente (mÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡s recientes primero)
      horariosEmpleado.sort((a, b) => new Date(b.fecha) - new Date(a.fecha));
      
      console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‹Å“Ãƒâ€šÃ‚Â¤ Horarios del empleado:', horariosEmpleado.length);
  ...
  // ============================================
  // FUNCIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN DE LOGIN - Usa tabla "usuarios" del sistema principal
  // ============================================
  const iniciarSesion = async (e) => {
    ...
    try {
      ...
      if (!usuarioData) {
        setErrorLogin('Documento o contraseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±a incorrectos');
        setCargando(false);
        return;
      }
      
      // Usuario encontrado - guardar sesiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n
      const datosUsuario = {
        id: usuarioData.id,
        nombre: usuarioData.nombre,
      ...
      setErrorLogin('Error al iniciar sesiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n');
    }
    setCargando(false);
  };
          ...
          <form onSubmit={iniciarSesion}>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', marginBottom: 6, color: '#333', fontWeight: 500 }}>
                NÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºmero de Documento
              </label>
              <input
                type="text"
                value={documento}
            ...
            >
              {cargando ? 'ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€šÃ‚Â³ Ingresando...' : 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€šÃ‚ÂÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Ingresar'}
            </button>
          </form>
          
          <div style={{ textAlign: 'center', marginTop: 20, color: '#999', fontSize: 12 }}>
            ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿Olvidaste tu contraseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±a? Contacta a Recursos Humanos
          </div>
        </div>
      </div>
    );
  ...
  // ============================================
  // MENÃƒÆ’Ã†â€™Ãƒâ€¦Ã‚Â¡ LATERAL
  // ============================================
  const menuItems = [
    { id: 'inicio', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€šÃ‚ÂÃƒâ€šÃ‚Â ', nombre: 'Inicio' },
    { id: 'desprendible', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢Ãƒâ€šÃ‚Â°', nombre: 'Desprendible de Pago' },
    { id: 'carta-laboral', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾', nombre: 'Carta Laboral' },
    { id: 'contrato', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¹', nombre: 'Contrato de Trabajo' },
    { id: 'horarios', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢Ãƒâ€šÃ‚Â', nombre: 'Mis Horarios' },
    { id: 'solicitudes', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â', nombre: 'Radicar Solicitud' },
    { id: 'reglamento', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“', nombre: 'Reglamento Interno' },
    { id: 'formatos', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â', nombre: 'Formatos' },
  ];
  ...
  const SeccionInicio = () => (
    <div>
      <div style={{
        ...
        />
        <div>
          <h2 style={{ margin: 0 }}>ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡Bienvenido, {empleado?.nombre || usuario?.nombre || 'Empleado'}!</h2>
  ...
  const SeccionDesprendible = () => {
    ...
    // FunciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n para calcular el rango de la quincena desde una fecha
    const getRangoQuincena = (fechaISO) => {
      ...
      // Calcular los dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­as de la quincena
    ...
    return (
      <div>
        <h2 style={{ color: '#c62828', marginBottom: 20 }}>ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢Ãƒâ€šÃ‚Â° Desprendible de Pago</h2>
        
        {!nominaSeleccionada ? (
          <div>
            <p style={{ color: '#666', marginBottom: 16 }}>
              Selecciona un perÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­odo para ver tu desprendible:
            </p>
            ...
            {nominas.length === 0 ? (
              <div style={{
                ...
                <div style={{ fontSize: 48, marginBottom: 16 }}>ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â­</div>
                <p style={{ color: '#666' }}>No hay desprendibles disponibles</p>
                <p style={{ color: '#999', fontSize: 12 }}>Los desprendibles aparecerÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡n aquÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­ cuando se procese la nÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³mina en el sistema.</p>
              </div>
            ) : (
          ...
          <div>
            <button
              ...
              className="no-print"
            >
              ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Ãƒâ€šÃ‚Â Volver
            </button>
            
            {/* DESPRENDIBLE - DiseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±o compacto para una sola hoja */}
            <div id="desprendible-print" style={{
              ...
              {horasTotales && (
                <div style={{ marginBottom: 12 }}>
                  <h4 style={{ color: '#1565c0', borderBottom: '1px solid #1565c0', paddingBottom: 4, margin: '0 0 8px', fontSize: 12 }}>
                    ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€šÃ‚Â° HORAS TRABAJADAS
                  </h4>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 12 }}>
                ...
                <div>
                  <h4 style={{ color: '#4caf50', borderBottom: '1px solid #4caf50', paddingBottom: 4, margin: '0 0 6px', fontSize: 12 }}>
                    ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢Ãƒâ€šÃ‚Âµ DEVENGADOS
                  </h4>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                    <tbody>
                      ...
                      {nominaSeleccionada.bonificacion > 0 && (
                        <tr>
                          <td style={{ padding: 4, borderBottom: '1px solid #eee' }}>BonificaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n</td>
                ...
                <div>
                  <h4 style={{ color: '#f44336', borderBottom: '1px solid #f44336', paddingBottom: 4, margin: '0 0 6px', fontSize: 12 }}>
                    ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â° DEDUCCIONES
                  </h4>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                    <tbody>
                      ...
                      {(nominaSeleccionada.descuentopension || nominaSeleccionada.descpension) > 0 && (
                        <tr>
                          <td style={{ padding: 4, borderBottom: '1px solid #eee' }}>PensiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n (4%)</td>
                      ...
                      {(nominaSeleccionada.descuentoprestamos || nominaSeleccionada.descprestamos) > 0 && (
                        <tr>
                          <td style={{ padding: 4, borderBottom: '1px solid #eee' }}>PrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©stamos</td>
              ...
              <div style={{
                ...
                <span style={{ fontSize: 14, fontWeight: 'bold' }}>ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢Ãƒâ€šÃ‚Â° NETO A PAGAR</span>
                <span style={{ fontSize: 20, fontWeight: 'bold' }}>{formatearMoneda(nominaSeleccionada.totalneto || nominaSeleccionada.netoapagar)}</span>
              </div>

              {/* Pie de pÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡gina */}
            ...
            <div style={{ marginTop: 16, textAlign: 'center' }} className="no-print">
              ...
              >
                ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“Ãƒâ€šÃ‚Â¨ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Imprimir Desprendible
              </button>
            </div>
          </div>
        )}
        
        {/* Estilos para impresiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n */}
        <style>{`
          @media print {
  ...
  // CARTA LABORAL - AutomÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡tica con datos de sede
  const SeccionCartaLaboral = () => {
    ...
    // FunciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n para convertir nÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºmero a letras
    const numeroALetras = (num) => {
      if (!num || num === 0) return 'CERO';
      const unidades = ['', 'UN', 'DOS', 'TRES', 'CUATRO', 'CINCO', 'SEIS', 'SIETE', 'OCHO', 'NUEVE'];
      const especiales = ['DIEZ', 'ONCE', 'DOCE', 'TRECE', 'CATORCE', 'QUINCE', 'DIECISÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°IS', 'DIECISIETE', 'DIECIOCHO', 'DIECINUEVE'];
      ...
      if (n === 1000000) return 'UN MILLÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN';
      ...
      if (n < 1000000000) {
        ...
        const millonesTexto = millones === 1 ? 'UN MILLÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN' : convertirCentena(millones) + ' MILLONES';
    ...
    const imprimirCarta = () => {
      ...
      // Campo correcto: fechaingreso (minÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºsculas, sin guiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n)
      const fechaIngreso = empleado?.fechaingreso || empleado?.fecha_ingreso || empleado?.fechaIngreso || '';
      // Campo correcto: tipocontrato (minÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºsculas, sin guiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n)
      const tipoContrato = empleado?.tipocontrato || empleado?.tipo_contrato || empleado?.tipoContrato || 'TÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rmino Indefinido';
      // Campo correcto: salariobase (minÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºsculas, sin guiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n)
      ...
      // Total: salario bÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡sico + auxilio de transporte
      const salarioTotal = salarioBase + AUXILIO_TRANSPORTE;
      
      console.log('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾ Carta Laboral - Salario:', salarioBase, '+ Aux:', AUXILIO_TRANSPORTE, '= Total:', salarioTotal);
      ...
      ventanaImpresion.document.write(`
        ...
        <html>
          <head>
            <title>CertificaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n Laboral</title>
            <meta charset="UTF-8">
            <style>
          ...
          <body>
            ...
            <div class="titulo">CERTIFICACIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN LABORAL</div>
            ...
            <div class="contenido">
              ...
              <p>El (la) SeÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±or(a) <strong>${nombreEmpleado.toUpperCase()}</strong>, identificado(a) con <strong>CÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©dula de CiudadanÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a ${documento}</strong>, labora en nuestra empresa${fechaIngreso ? ` desde el <strong>${new Date(fechaIngreso).toLocaleDateString('es-CO', { day: 'numeric', month: 'long', year: 'numeric' })}</strong>` : ''}, con un contrato <strong>${tipoContrato}</strong>, desempeÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±ando el cargo de <strong>${cargo.toUpperCase()}</strong>${salarioBase > 0 ? `, devengando un salario bÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡sico mensual de <strong>${formatearMoneda(salarioBase)}</strong> mÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡s auxilio de transporte de <strong>${formatearMoneda(AUXILIO_TRANSPORTE)}</strong>, para un total devengado de <strong>${formatearMoneda(salarioTotal)}</strong> (${numeroALetras(salarioTotal)} PESOS M/CTE)` : ''}.</p>
              
              <p>La presente certificaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n se expide a solicitud del interesado para los fines que estime conveniente.</p>
            </div>
            ...
            <div class="firma">
              ...
              ${direccionSede ? `<div class="cargo-firma">DirecciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n: ${direccionSede}</div>` : ''}
              ${telefonoSede ? `<div class="cargo-firma">TelÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©fono: ${telefonoSede}</div>` : ''}
            </div>
          </body>
        </html>
      `);
    ...
    // Campos correctos segÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºn estructura tabla
    const fechaIngresoEmpleado = empleado?.fechaingreso || empleado?.fecha_ingreso || '';
    const tipoContratoEmpleado = empleado?.tipocontrato || empleado?.tipo_contrato || 'TÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rmino Indefinido';

    return (
      <div>
        <h2 style={{ color: '#c62828', marginBottom: 20 }}>ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾ CertificaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n Laboral</h2>
        ...
        <div id="carta-print" style={{
          ...
          <h4 style={{ textAlign: 'center', margin: '24px 0', textDecoration: 'underline' }}>
            CERTIFICACIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN LABORAL
          </h4>
          ...
          <div style={{ lineHeight: 1.8, textAlign: 'justify' }}>
            ...
            <p>
              El (la) SeÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±or(a) <strong>{nombreCompleto.toUpperCase()}</strong>, 
              identificado(a) con <strong>CÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©dula de CiudadanÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a {empleado?.documento || usuario?.usuario}</strong>, 
              ...
              desempeÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±ando el cargo de <strong>{(empleado?.cargo || 'COLABORADOR').toUpperCase()}</strong>
              {salarioEmpleado > 0 && (
                <>, devengando un salario bÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡sico mensual de <strong>{formatearMoneda(salarioEmpleado)}</strong> mÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡s auxilio de transporte de <strong>{formatearMoneda(AUXILIO_TRANSPORTE)}</strong>, para un total devengado de <strong>{formatearMoneda(salarioTotal)}</strong></>
              )}.
            </p>
            
            <p style={{ marginTop: 16 }}>
              La presente certificaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n se expide a solicitud del interesado para los fines que estime conveniente.
            </p>
          </div>
        ...
        {/* BotÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n imprimir */}
        <div style={{ marginTop: 24, textAlign: 'center' }}>
          ...
          >
            ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“Ãƒâ€šÃ‚Â¨ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Imprimir CertificaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n
          </button>
          <p style={{ marginTop: 12, fontSize: 12, color: '#666' }}>
            La certificaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n se generarÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ con los datos actuales y podrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡s imprimirla o guardarla como PDF.
          </p>
        </div>
      </div>
    );
  ...
  const SeccionContrato = () => {
    ...
    // FunciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n para generar e imprimir el contrato como PDF - IGUAL AL SISTEMA ORIGINAL
    const imprimirContrato = () => {
      ...
      // Variables de gÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©nero
      ...
      const ellaEl = esEmpleadoMujer ? "ella" : "ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©l";
      
      // Logo Big Burguer en Base64 para que aparezca siempre en la impresiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n
      ...
      win.document.write(`
        <html>
          <head>
            <title>${datos.tipoContrato === "Fijo" || datos.tipoContrato === "TÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rmino Fijo"
              ? "Contrato Individual de Trabajo a TÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rmino Fijo"
              : "Contrato Individual de Trabajo a TÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rmino Indefinido"} - ${datos.nombreTrabajador || ''}</title>
            <style>
              @page {
                size: letter;
          ...
          <body>
            <div class="header-container">
              ...
              <h1>CONTRATO INDIVIDUAL DE TRABAJO A TÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°RMINO ${datos.tipoContrato === "Fijo" || datos.tipoContrato === "TÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rmino Fijo" ? "FIJO" : "INDEFINIDO"}</h1>
            </div>
            ...
            <table class="tabla-datos">
              ...
              <tr><td class="label">DIRECCIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN DEL EMPLEADOR</td><td class="valor">${datos.direccionEmpleador || ''}</td></tr>
              <tr><td class="label">TELÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°FONO</td><td class="valor">${datos.telefonoEmpleador || ''}</td></tr>
              <tr><td class="label">REPRESENTANTE LEGAL</td><td class="valor">${datos.representanteLegal || ''}</td></tr>
              <tr><td class="label">${datos.tipoDocRepresentante ? datos.tipoDocRepresentante.toUpperCase() : "CÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°DULA DE CIUDADANÃƒÆ’Ã†â€™Ãƒâ€šÃ‚ÂA"}</td><td class="valor">${datos.cedulaRepresentante || ''}</td></tr>
              ...
              <tr><td class="label">${datos.tipoDocTrabajador ? datos.tipoDocTrabajador.toUpperCase() : "CÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°DULA DE CIUDADANÃƒÆ’Ã†â€™Ãƒâ€šÃ‚ÂA"}</td><td class="valor">${datos.cedulaTrabajador || ''}</td></tr>
              <tr><td class="label">LUGAR Y FECHA NACIMIENTO</td><td class="valor">${datos.lugarFechaNacimiento || ''}</td></tr>
              <tr><td class="label">DIRECCIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN</td><td class="valor">${datos.direccionTrabajador || ''}</td></tr>
              <tr><td class="label">TELÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°FONO</td><td class="valor">${datos.telefonoTrabajador || ''}</td></tr>
              ...
              <tr><td class="label">REMUNERACIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN SALARIAL MENSUAL</td><td class="valor">${datos.remuneracion || ''} (${datos.remuneracionLetras || ''} PESOS M/CTE)</td></tr>
              <tr><td class="label">PERÃƒÆ’Ã†â€™Ãƒâ€šÃ‚ÂODO DE PAGO</td><td class="valor">${datos.periodoPago || ''}</td></tr>
              <tr><td class="label">FECHA INICIACIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN DE LABORES</td><td class="valor">${datos.fechaInicio || ''}</td></tr>
              <tr><td class="label">FECHA DE TERMINACIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN DE LABORES</td><td class="valor">${datos.fechaTerminacion || ''}</td></tr>
              <tr><td class="label">LUGAR DE TRABAJO</td><td class="valor">${datos.lugarTrabajo || ''}</td></tr>
              <tr><td class="label">LUGAR DE CONTRATACIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN</td><td class="valor">${datos.lugarContratacion || ''}</td></tr>
            </table>

            <p class="intro-text">Entre el EMPLEADOR y ${elLaTrabajador} ${trabajadorNombre}, de las condiciones ya dichas, identificados como aparece al pie de sus firmas, se ha celebrado el presente contrato individual de trabajo a tÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rmino ${datos.tipoContrato === "Fijo" || datos.tipoContrato === "TÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rmino Fijo" ? "fijo" : "indefinido"}, regido ademÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡s por las siguientes <strong>CLÃƒÆ’Ã†â€™Ãƒâ€šÃ‚ÂUSULAS:</strong></p>
            
            <div class="clausula"><span class="clausula-titulo">PRIMERA: OBJETO.</span> EL EMPLEADOR contrata los servicios personales de ${elLaTrabajador} ${trabajadorNombre} en el cargo reseÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±ado y ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©ste se obliga: a) a poner al servicio del EMPLEADOR toda su capacidad normal de trabajo en el desempeÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±o de las funciones propias del oficio mencionado y en las labores descritas en el literal f de la presente clÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡usula y complementarias del mismo, de conformidad con las ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³rdenes e instrucciones que le imparta EL EMPLEADOR directamente o travÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©s de sus representantes. Las funciones serÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡n detalladas en Anexo al presente Contrato; b) a prestar sus servicios en forma exclusiva a EL EMPLEADOR, es decir, a no prestar directa ni indirectamente servicios laborales a otros empleadores, ni trabajar por cuenta propia en el mismo oficio, durante la vigencia de este contrato; y c) a guardar absoluta reserva y confidencialidad sobre los hechos, documentos fÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­sicos y/o electrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³nicos, informaciones y en general, sobre todos los asuntos y materias que lleguen a su conocimiento por causa o por ocasiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de su contrato de trabajo y aun despuÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©s dos (2) aÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±os de liquidado el mismo. En caso de incumplimiento de la presente obligaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n, ${elLaTrabajador} ${trabajadorNombre} responderÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ legalmente por los daÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±os y/o perjuicios que se causen a la empresa, de conformidad con las normas vigentes en la materia. d) a reportar cualquier orden, solicitud, o novedad que reciba de su jefe inmediato o de cualquier compaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±ero o colaborador, tendiente a realizar o encubrir actos fraudulentos o ilÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­citos que afecten de cualquier forma a EL EMPLEADOR. e) Dar cumplimiento a las polÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­ticas que estipule el Empleador, los cuales constan en los anexos que forman parte integral de este contrato. f) ${elLaTrabajador} ${trabajadorNombre} desempeÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±arÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ las funciones tales como: Presentar el menÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âº, conocer los ingredientes y las preparaciones, sugerir platos, presentar las recomendaciones del dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a y las bebidas disponibles, ser enlace entre la cocina y el cliente, debe anotar pedidos y entregarlos al comando de la cocina, cerciorarse que los platos hayan sido preparados de forma correcta, en caso de que el comensal haya hecho una peticiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n especial, mantener comunicaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n continua con los clientes, prestar atenciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n a las reacciones de los clientes y canalizar quejas o sugerencias que busquen mejorar el servicio, mantener las mesas limpias y desinfectadas antes y despuÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©s de su uso por parte del cliente, y demÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡s indicaciones que se le asignen o se le requieran, demÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡s instrucciones dadas por el EMPLEADOR.</div>

            <div class="clausula"><span class="clausula-titulo">SEGUNDA: REMUNERACIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN.</span> ${elLaTrabajador} ${trabajadorNombre} devengarÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ una remuneraciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de UN (1) SALARIO MÃƒÆ’Ã†â€™Ãƒâ€šÃ‚ÂNIMO LEGAL MENSUAL VIGENTE, equivalente actualmente a la suma de ${datos.remuneracionLetras || ''} PESOS M/CTE (${datos.remuneracion || ''}).<div class="paragrafo"><strong>PARÃƒÆ’Ã†â€™Ãƒâ€šÃ‚ÂGRAFO PRIMERO: SALARIO ORDINARIO.</strong> Dentro del salario ordinario se encuentra incluida la remuneraciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de los descansos dominicales y festivos de que tratan los CapÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­tulos I, II y III del TÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­tulo VII del C.S.T. De igual manera se aclara y se conviene que en los casos en que ${elLaTrabajador} ${trabajadorNombre} devengue comisiones o cualquiera otra modalidad de salario variable, el 82.5% de dichos ingresos constituye remuneraciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de la labor realizada, y el 17.5% restante estarÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ destinado a remunerar el descanso en los dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­as dominicales y festivos de que tratan los CapÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­tulos I y II del TÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­tulo VIII del C.S.T.</div><div class="paragrafo"><strong>PARÃƒÆ’Ã†â€™Ãƒâ€šÃ‚ÂGRAFO SEGUNDO: SALARIO INTEGRAL.</strong> En la eventualidad en que ${elLaTrabajador} ${trabajadorNombre} devengue salario integral, se entiende de conformidad con el numeral 2 del artÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­culo 132 del C.S.T, subrogado por el artÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­culo 18 de la ley 50/90, que dentro del salario integral convenido se encuentra incorporado el factor prestacional de ${elLaTrabajador} ${trabajadorNombre}, el cual no serÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ inferior al 30% del salario antes mencionado.</div><div class="paragrafo"><strong>PARÃƒÆ’Ã†â€™Ãƒâ€šÃ‚ÂGRAFO TERCERO:</strong> Las partes acuerdan que en los casos en que se le reconozcan a ${elLaTrabajador} ${trabajadorNombre} beneficios diferentes al salario por concepto de alimentaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n, comunicaciones, habitaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n o vivienda, transporte, vestuario, auxilios en dinero o en especie o bonificaciones ocasionales, ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©sos no se considerarÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡n como factor constitutivo de salario y no se tendrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡n en cuenta como factor prestacional para la liquidaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de acreencias laborales, ni para el pago de aportes parafiscales y cotizaciones a la seguridad social, de conformidad con los Arts. 15 y 16 de la ley 50 de 1990, en concordancia el Art. 17 de la ley 344 de 1996.</div></div>

            <div class="clausula"><span class="clausula-titulo">TERCERA: DURACIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN DEL CONTRATO.</span> ${datos.tipoContrato === "Fijo" || datos.tipoContrato === "TÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rmino Fijo" ? "La duraciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n del presente contrato serÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ por el tÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rmino establecido en la parte inicial del presente documento, contado a partir de la fecha de iniciaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de labores. No obstante, si antes de la fecha de vencimiento del tÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rmino estipulado, ninguna de las partes avisare por escrito a la otra su determinaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de no prorrogar el contrato, con una antelaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n no inferior a treinta (30) dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­as, ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©ste se entenderÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ renovado por un perÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­odo igual al inicialmente pactado." : "La duraciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n del presente contrato serÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ de manera indefinida, periodo entre la fecha de iniciaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n del contrato establecida en la parte inicial del presente documento y terminarÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ segÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºn las razones dispuestas por la ley."}</div>

            <div class="clausula"><span class="clausula-titulo">CUARTA: TRABAJO NOCTURNO, SUPLEMENTARIO, DOMINICAL Y/O FESTIVO.</span> Todo trabajo nocturno, suplementario o en horas extras, y todo trabajo en dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a domingo o festivo en los que legalmente debe concederse descanso, se remunerarÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ conforme los dispone expresamente la ley, salvo acuerdo en contrario contenido en convenciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n, pacto colectivo o laudo arbitral. Para el reconocimiento y pago del trabajo suplementario, nocturno, dominical o festivo, EL EMPLEADOR o sus representantes deberÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡n haberlo autorizado previamente y por escrito.</div>

            <div class="clausula"><span class="clausula-titulo">QUINTA: JORNADA DE TRABAJO.</span> ${elLaTrabajador} ${trabajadorNombre} se obliga a laborar la jornada mÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡xima legal, salvo acuerdo especial, cumpliendo con los turnos y horarios que seÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±ale EL EMPLEADOR, quien podrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ cambiarlos o ajustarlos cuando lo estime conveniente sin que ello se considere una desmejora en las condiciones laborales ${esEmpleadoMujer ? "de LA TRABAJADORA" : "del TRABAJADOR"}.</div>

            <div class="clausula"><span class="clausula-titulo">SEXTA: PERIODO DE PRUEBA.</span> Los 60 dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­as iniciales del contrato se considera como periodo de prueba sin que exceda los lÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­mites permitidos a partir de la fecha de inicio y por consiguiente, cualquiera de las partes podrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ terminar el contrato unilateralmente, en cualquier momento durante dicho periodo.</div>

            <div class="clausula"><span class="clausula-titulo">SÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°PTIMA: TERMINACIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN UNILATERAL.</span> Son justas causas para dar terminado unilateralmente este contrato, por cualquiera de las partes, las enumeradas en el Art. 62 del C.S.T., modificado por el Art. 7ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âª del Decreto 2351 de 1965 y ademÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡s, por parte de EL EMPLEADOR, las faltas que para el efecto se califiquen como graves en reglamentos, manuales, instructivos y demÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡s documentos que contengan reglamentaciones, ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³rdenes, instrucciones o prohibiciones de carÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡cter general o particular.<div class="paragrafo"><strong>PARÃƒÆ’Ã†â€™Ãƒâ€šÃ‚ÂGRAFO:</strong> Al finalizar el contrato de trabajo por cualquier concepto, ${elLaTrabajador} ${trabajadorNombre} autoriza descontar de su liquidaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n final de prestaciones sociales el valor correspondiente a los faltantes y/o deterioro anormal de elementos puestos bajo su responsabilidad.</div></div>

            <div class="clausula"><span class="clausula-titulo">OCTAVA: PROPIEDAD INTELECTUAL.</span> Las partes acuerdan que todas las invenciones, descubrimientos y trabajos originales concebidos o hechos por ${elLaTrabajador} ${trabajadorNombre} en vigencia del presente contrato pertenecerÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡n a EL EMPLEADOR, por lo cual ${elLaTrabajador} ${trabajadorNombre} se obliga a informar a EL EMPLEADOR, de forma inmediata, sobre la existencia de dichas invenciones y/o trabajos originales.</div>

            <div class="clausula"><span class="clausula-titulo">NOVENA: MODIFICACIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN DE LAS CONDICIONES LABORALES.</span> ${elLaTrabajador} ${trabajadorNombre} acepta desde ahora expresamente todas las modificaciones de sus condiciones laborales determinadas por EL EMPLEADOR en ejercicio de su poder subordinante, tales como el horario de trabajo, el lugar de prestaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n del servicio y el cargo u oficio y/o funciones, siempre que tales modificaciones no afecten su honor, dignidad o sus derechos mÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­nimos, ni impliquen desmejoras sustanciales o graves perjuicios para ${ellaEl}.</div>

            <div class="clausula"><span class="clausula-titulo">DÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°CIMA: DIRECCIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN ${esEmpleadoMujer ? "DE LA TRABAJADORA" : "DEL TRABAJADOR"}.</span> ${elLaTrabajador} ${trabajadorNombre} se compromete a informar por escrito y de manera inmediata a EL EMPLEADOR cualquier cambio en su direcciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de residencia, teniÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©ndose en todo caso como suya, la ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºltima direcciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n registrada en su hoja de vida.</div>

            <div class="clausula"><span class="clausula-titulo">DÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°CIMA PRIMERA: EFECTOS.</span> El presente contrato reemplaza en su integridad y deja sin efecto cualquiera otro contrato, verbal o escrito, celebrado entre las partes con anterioridad, pudiendo las partes convenir por escrito modificaciones al mismo, las que formarÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡n parte integral de este contrato.</div>

            <div class="clausula"><span class="clausula-titulo">DÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°CIMA SEGUNDA: USO DE INTERNET.</span> ${elLaTrabajador} ${trabajadorNombre}, en razÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de sus funciones, tendrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ acceso a Internet. ${elLaTrabajador} ${trabajadorNombre} se compromete a realizar un uso adecuado del Internet desde su computador o dispositivo mÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³vil o cualquier otro dispositivo de la empresa con conexiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n a Internet. Se abstiene de usarlo para el ingreso a pÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ginas que no sean del desarrollo de sus funciones.</div>

            <div class="clausula"><span class="clausula-titulo">DÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°CIMA TERCERA: HABEAS DATA.</span> Los datos consignados en el presente Contrato serÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡n tratados de acuerdo a lo establecido en la Ley 1581 de 2012, en el Decreto 1377 de 2013 y cualquier otra normatividad en lo que respecta a la protecciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de la informaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n.</div>

            <div class="clausula"><span class="clausula-titulo">DÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°CIMA CUARTA: AUTORIZACIÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œN DESCUENTOS.</span> ${elLaTrabajador} ${trabajadorNombre} autoriza desde ahora al EMPLEADOR para que, de sus salarios, prestaciones sociales e indemnizaciones, le descuente, durante la vigencia del contrato o al momento de la terminaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n del mismo por cualquier causa, las sumas de dinero que por cualquier motivo le llegare a adeudar.</div>

            <div class="clausula"><span class="clausula-titulo">DÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°CIMA QUINTA: OBLIGACIONES ESPECIALES DE CONFIDENCIALIDAD ${esEmpleadoMujer ? "DE LA TRABAJADORA" : "DEL TRABAJADOR"}.</span> ${elLaTrabajador} ${trabajadorNombre} se obliga a:<br/>a. Guardar absoluta confidencialidad respecto a: procedimientos, mÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©todos, caracterÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­sticas, lista de clientes, fÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³rmulas de productos y similares, al igual que claves de seguridad, suministros, software, base de datos de cualquier ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­ndole, valores de bienes y servicios, informaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n tÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©cnica, financiera, econÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³mica o comercial del contratante o sus clientes.<br/>b. No ejercer actos de competencia desleal frente a ${datos.nombreEmpleador || ''}.<br/>c. Adoptar todas las precauciones necesarias y apropiadas para guardar la confidencialidad de la informaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n.<br/>d. Devolver inmediatamente a la terminaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de su contrato: la lista de clientes, claves, bases de datos, equipos, informaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n tÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©cnica, y demÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡s que tenga del empleador.<div class="paragrafo"><strong>PARÃƒÆ’Ã†â€™Ãƒâ€šÃ‚ÂGRAFO:</strong> El incumplimiento u omisiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de cualquiera de las obligaciones aquÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­ acordadas no solo es causal de terminaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de los vÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­nculos laborales existentes entre las partes, sino que podrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a conllevar a iniciar acciones judiciales en contra ${esEmpleadoMujer ? "de la trabajadora" : "del trabajador"} por los perjuicios materiales e inmateriales que cause.</div></div>

            <p style="margin-top: 25px;">Para constancia se firma en dos ejemplares del mismo tenor y valor, ante testigos en la ciudad y fecha que se indican a continuaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n:</p>
            <p style="margin: 15px 0;"><strong>CIUDAD:</strong> ${datos.ciudad || ''} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>FECHA:</strong> ${datos.fechaFirma || ''}</p>
            
            <div class="firma-container">
              <div class="firma-box"><div class="espacio-firma"></div><div class="linea-firma"></div><div class="nombre-firma">EMPLEADOR</div><div class="nombre-firma">${datos.representanteLegal || ''}</div><div class="cedula-firma">${datos.tipoDocRepresentante || "CÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©dula de CiudadanÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a"} ${datos.cedulaRepresentante || ''}</div><div class="cedula-firma">Representante Legal</div></div>
              <div class="firma-box"><div class="espacio-firma"></div><div class="linea-firma"></div><div class="nombre-firma">${trabajadorNombre}</div><div class="nombre-firma">${datos.nombreTrabajador || ''}</div><div class="cedula-firma">${datos.tipoDocTrabajador || "CÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©dula de CiudadanÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a"} ${datos.cedulaTrabajador || ''}</div></div>
            </div>
    ...
    if (cargandoContrato) {
      return (
        <div style={{ textAlign: 'center', padding: 40 }}>
          <div style={{ fontSize: 40 }}>ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€šÃ‚Â³</div>
    ...
    return (
      <div>
        <h2 style={{ color: '#c62828', marginBottom: 20 }}>ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¹ Contrato de Trabajo</h2>
        ...
        {contrato ? (
          <div>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                <div>
                  ...
                  <p style={{ margin: '4px 0', color: '#666', fontSize: 13 }}>
                    {contrato.datos?.tipoContrato || contrato.tipocontrato || 'TÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©rmino Indefinido'}
                  </p>
                </div>
                <div style={{
                  padding: '6px 12px',
                  ...
                  ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã¢â‚¬Å“ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ Vigente
                </div>
              </div>
              ...
              {/* BotÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n para imprimir/descargar */}
              <div style={{ textAlign: 'center' }}>
                ...
                >
                  ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾ Ver / Imprimir Contrato (PDF)
                </button>
                <p style={{ color: '#666', fontSize: 12, marginTop: 10 }}>
                  Se abrirÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ una ventana con tu contrato listo para imprimir o guardar como PDF
                </p>
              </div>
            </div>
            
            {/* Fecha de generaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n */}
          ...
          <div style={{
            ...
            <div style={{ fontSize: 60, marginBottom: 16 }}>ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â­</div>
            <h3 style={{ color: '#e65100' }}>Contrato no disponible</h3>
            <p style={{ color: '#666' }}>
              Tu contrato aÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºn no ha sido generado en el sistema.<br />
              Por favor, contacta al ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡rea de Recursos Humanos.
            </p>
          </div>
        )}
      </div>
  ...
  const SeccionHorarios = () => {
    ...
    const diasSemanaCorto = ['Dom', 'Lun', 'Mar', 'MiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©', 'Jue', 'Vie', 'SÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡b'];
    const meses = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'];
    
    // Festivos de Colombia 2026 (Ley 51 de 1983)
    const festivosColombia2026 = {
      '2026-01-01': 'AÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±o Nuevo',
      '2026-01-12': 'DÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a de los Reyes Magos',
      '2026-03-23': 'DÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a de San JosÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©',
      '2026-04-02': 'Jueves Santo',
      '2026-04-03': 'Viernes Santo',
      '2026-05-01': 'DÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a del Trabajo',
      '2026-05-18': 'AscensiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n del SeÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±or',
      '2026-06-08': 'Corpus Christi',
      '2026-06-15': 'Sagrado CorazÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n',
      '2026-06-29': 'San Pedro y San Pablo',
      '2026-07-20': 'DÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a de la Independencia',
      '2026-08-07': 'Batalla de BoyacÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡',
      '2026-08-17': 'AsunciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n de la Virgen',
      '2026-10-12': 'DÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a de la Raza',
      '2026-11-02': 'Todos los Santos',
      '2026-11-16': 'Independencia de Cartagena',
      '2026-12-08': 'Inmaculada ConcepciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n',
      '2026-12-25': 'Navidad',
      // 2025
      '2025-12-08': 'Inmaculada ConcepciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n',
      '2025-12-25': 'Navidad',
    };
        ...
        try {
          ...
          if (data) {
            ...
            data.forEach(semana => {
              ...
              // Eventos por dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a
    ...
    // FunciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n para convertir hora 24h a formato AM/PM
    ...
    // Crear mapa de horarios por fecha para acceso rÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡pido
    ...
    const generarSemanasDelMes = (year, month) => {
      ...
      // Empezar desde el domingo de la semana del primer dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a
      ...
      return (
        <div style={{ marginBottom: 30 }}>
          <h3 style={{ 
            ...
            ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â  {meses[month]} {year}
          </h3>
          
          {/* Encabezados de dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­as */}
          <div style={{ 
            display: 'grid', 
              ...
              {semana.map((fecha, diaIdx) => {
                ...
                return (
                  <div key={diaIdx} style={{
                    ...
                    {/* NÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºmero del dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a */}
                    <div style={{
                      fontWeight: '900',
                      fontSize: 22,
                    ...
                    {esFestivo && esDelMes && (
                      <div style={{
                        ...
                        ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€¦Ã‚Â½ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â° {festivo}
                      </div>
                    )}
                      ...
                      <div style={{ fontSize: 12 }}>
                        {horario.es_descanso ? (
                          <div style={{
                            ...
                            ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€¦Ã¢â‚¬â„¢Ãƒâ€šÃ‚Â´ Descanso
                          </div>
                          ...
                          <div>
                            ...
                            <div style={{ 
                              ...
                              ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã‚Â¡Ãƒâ€šÃ‚Â¡ Partido
                            </div>
                          </div>
                        ) : (
                          <div style={{
    ...
    return (
      <div>
        <h2 style={{ color: '#c62828', marginBottom: 10 }}>ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢Ãƒâ€šÃ‚Â Mis Horarios</h2>
        
        <p style={{ color: '#666', marginBottom: 20, fontSize: 14 }}>
          ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ Calendario de horarios - Mes actual y mes anterior
        </p>
        ...
        {horarios.length === 0 ? (
          <div style={{
            ...
            <div style={{ fontSize: 60, marginBottom: 16 }}>ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦</div>
            <h3>No hay horarios programados</h3>
            <p style={{ color: '#666' }}>
              AÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºn no tienes horarios asignados.<br />
              Los horarios aparecerÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡n aquÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­ cuando sean programados por tu supervisor.
            </p>
          </div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: '500' }}>ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€¦Ã‚Â½ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â° Festivo</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
  ...
  const SeccionSolicitudes = () => {
    ...
    const tiposSolicitud = [
      { id: 'permiso', nombre: 'Permiso', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¹' },
      { id: 'vacaciones', nombre: 'Vacaciones', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€šÃ‚ÂÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â' },
      { id: 'licencia', nombre: 'Licencia', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¹' },
      { id: 'cambio_horario', nombre: 'Cambio de Horario', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢Ãƒâ€šÃ‚Â' },
      { id: 'certificado', nombre: 'Certificado Laboral', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾' },
      { id: 'otro', nombre: 'Otra Solicitud', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â' },
    ];
    ...
    const enviarSolicitud = async (e) => {
      ...
      try {
        ...
        if (!error) {
          alert('ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã¢â‚¬Å“ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ Solicitud enviada correctamente');
          setModo('lista');
          setTipoSolicitud('');
          setDescripcion('');
          setFechaInicio('');
          ...
          alert('ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦Ã¢â‚¬â„¢ Error al enviar la solicitud');
        }
      } catch (error) {
        console.error('Error:', error);
        alert('ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦Ã¢â‚¬â„¢ Error al enviar la solicitud');
    ...
    return (
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h2 style={{ color: '#c62828', margin: 0 }}>ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â Solicitudes</h2>
          {modo === 'lista' && (
            <button
            ...
            >
              ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã‚Â¾ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ Nueva Solicitud
            </button>
          )}
        </div>
        ...
        {modo === 'nueva' ? (
          <div style={{
            ...
            <form onSubmit={enviarSolicitud}>
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: 'block', marginBottom: 8, fontWeight: 'bold' }}>
                  DescripciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n / Motivo *
                </label>
                <textarea
                  value={descripcion}
              ...
              >
                {enviando ? 'ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€šÃ‚Â³ Enviando...' : 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â¤ Enviar Solicitud'}
              </button>
            </form>
          </div>
        ) : (
          <div>
            {solicitudes.length === 0 ? (
              <div style={{
                ...
                <p style={{ color: '#666' }}>
                  AÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºn no has radicado ninguna solicitud.
                </p>
              </div>
            ) : (
              <div style={{ display: 'grid', gap: 12 }}>
                {solicitudes.map(sol => {
                  ...
                  const tiposSolicitudMap = {
                    permiso: { nombre: 'Permiso', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¹' },
                    vacaciones: { nombre: 'Vacaciones', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€šÃ‚ÂÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â' },
                    licencia: { nombre: 'Licencia', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¹' },
                    cambio_horario: { nombre: 'Cambio de Horario', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢Ãƒâ€šÃ‚Â' },
                    certificado: { nombre: 'Certificado Laboral', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾' },
                    otro: { nombre: 'Otra Solicitud', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â' }
                  };
                  const tipo = tiposSolicitudMap[sol.tipo] || { nombre: sol.tipo, icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â' };
  ...
  const SeccionReglamento = () => {
    ...
    return (
      <div>
        <h2 style={{ color: '#c62828', marginBottom: 20 }}>ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Reglamento Interno de Trabajo</h2>
        ...
        {reglamentoUrl ? (
          <div style={{
            ...
            <div style={{ fontSize: 60, marginBottom: 16 }}>ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“</div>
            ...
            >
              ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â¥ Descargar Reglamento
            </a>
          </div>
        ) : (
          <div style={{
            ...
            <p style={{ color: '#666' }}>
              El reglamento interno aÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºn no ha sido cargado al sistema.<br />
              Por favor, contacta al ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡rea de Recursos Humanos.
            </p>
          </div>
        )}
        
        {/* InformaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n bÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡sica */}
        <div style={{
          marginTop: 24,
          padding: 20,
          backgroundColor: 'white',
          ...
          <h4 style={{ color: '#c62828', marginBottom: 16 }}>ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¹ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â InformaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n Importante</h4>
  ...
  const SeccionFormatos = () => {
    ...
    const cargarFormatos = async () => {
      try {
        ...
        setFormatosDisponibles([
          { id: 'formato_permiso', nombre: 'Formato Solicitud de Permiso', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â' },
          { id: 'formato_vacaciones', nombre: 'Formato Solicitud de Vacaciones', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€šÃ‚ÂÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â' },
          { id: 'formato_licencia', nombre: 'Formato Solicitud de Licencia', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¹' },
          { id: 'formato_incapacidad', nombre: 'Formato Reporte de Incapacidad', icono: 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€šÃ‚ÂÃƒâ€šÃ‚Â¥' },
          { id: 'formato_horas_extra', nombre: 'Formato AutorizaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n Horas Extra', icono: 'ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€šÃ‚Â°' },
        ]);
      }
    };
    ...
    return (
      <div>
        <h2 style={{ color: '#c62828', marginBottom: 20 }}>ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â Formatos</h2>
        
        <p style={{ color: '#666', marginBottom: 20 }}>
          Descarga los formatos que necesites para tus trÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡mites internos.
        </p>
        ...
        {formatosDisponibles.length === 0 ? (
          <div style={{
            ...
            <p style={{ color: '#666' }}>
              Los formatos aparecerÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡n aquÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­ cuando sean cargados por Recursos Humanos.
            </p>
          </div>
        ) : (
          <div style={{ display: 'grid', gap: 12 }}>
            {formatosDisponibles.map(formato => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ fontSize: 32 }}>{formato.icono || 'ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾'}</span>
                  <div>
                ...
                {formato.url ? (
                  ...
                  >
                    ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒâ€šÃ‚Â¥ Descargar
                  </a>
                ) : (
        ...
        <div style={{
          ...
          <p style={{ margin: 0, color: '#2e7d32' }}>
            ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢Ãƒâ€šÃ‚Â¡ <strong>Tip:</strong> Los formatos descargados pueden ser llenados digitalmente o impresos para diligenciar a mano.
          </p>
        </div>
      </div>
    );
  ...
  // Renderizar secciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n activa
  const renderSeccion = () => {
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f5f5' }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          >
            ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€¦Ã‚Â¡Ãƒâ€šÃ‚Âª Salir
          </button>
        </div>
      </header>
